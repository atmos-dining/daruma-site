/**
 * common.js — 焼肉はひとくちめ だるま 共通スクリプト
 */
...
(function () {
  ...
  function initStickyHeader() {
    ...
    const RANGE        = 200; // このpx分スクロールで完全にコンパクトになる
    const LOGO_COMPACT = 56;  // コンパクト時のロゴ高さ

    // ボタンの初期スタイルを設定（高さをJSで制御）
    if (reserveBtn) {
    ...
    function onScroll() {
      ...
      // ① header-inner 上下padding: 16 → 0（ボタンが端まで届く）
      ...
      // ③ ボタン高さ = コンパクト時のヘッダー高さに合わせる
      //    normal時: 46px（自然なサイズ）→ compact時: 56px（ロゴと同じ高さ = ヘッダー全体）
      if (reserveBtn) {
